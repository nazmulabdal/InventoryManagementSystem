...
<body>
    ...
    $date=date("d-m-y");
    @endphp
    <h1 style="text-align: center;">Expense of {{ $date }}</h1>
    <table>
        <tr>
            <th>Supplier Name</th>
            <th>Details</th>
        ...
        @foreach ($today_expense_data as $row)
            <tr>
                <td>{{ $row->name }}</td>
                <td>{{ $row->details }}</td>
                <td>{{ $row->quantity }}</td>
    ...
    @endphp
    <br>
    <h4 style="color:darkslateblue; font-size: 22px;" class="pull-right">Total Expense : {{ $expense }} BDT </h4>
    <br> <br> <br>
    <div class="hidden-print">
        <div class="pull-right">
